@extends('layouts.dashboard.app', ['select2' => true, 'snippts' => true, 'jqueryUI' => true, 'modals' => ['customer', 'store', 'chequeInput'], 'datatables' => []])
								...
								<td>
									...
									<div class="input-group">
										...
										<input type="number" class="form-control input input-quantity" name="quantities[]"
											value="0" min="0" max="{{ $item->quantity - $item->items->sum('quantity') }}" />
									</div>
								</td>
							</tr>
								@php
		...
		@else
			<div class="row">
				...
				<div class="col-md-4 col-xs-12">
					<div class="form-group">
						@if ($bill)
							<label>بيع مباشر من مورد</label>
							<span class="form-control">{{ $bill->supplier->name }}</span>
						@else
				...
				<div class="col-md-4 col-xs-12">
					<div class="form-group">
						<label>العميل</label>
						<div class="input-group">
							<select class="form-control select2-single" id="bonus" onChange="calcbouns(this.options[this.selectedIndex].getAttribute('bonus'))" name="customer_id">
								<option> حدد  </option>
								@foreach ($customers as $customer)
									<option onClick="test()" bonus="{{$customer->bouns}}" value="{{ $customer->id }}">{{ $customer->name }}</option>
								@endforeach
							</select>
							@permission('customers-create')
							<div class="input-group-btn">
								<button type="button" class="btn btn-success showCustomerModal"><i class="fa fa-plus"></i></button>
							</div>
							@endpermission
						</div>
					</div>
						...
						<tbody>
							@if ($bill)
								...
								<tr>
									...
									<td>{{ $item->unitName() }}</td>
									<td>
										<input type="hidden" name="billItems[]" value="{{ $item->id }}" />
										<input type="hidden" name="units[]" value="{{ $item->item_store_unit_id }}" />
										<input type="number" min="1" max="{{ $item->quantity - $item->items->sum('quantity') }}"
											class="form-control input input-quantity" name="quantities[]" value="{{ $item->quantity - $item->items->sum('quantity') }}" />
									</td>
									<td>
										<input type="number" min="0" step="0.01" class="form-control input input-price price-purchase"
											name="purchases[]" value="{{ $item->price }}" />
									</td>
									<td>
			...
			<div class="box box-success">
				...
				<div class="box-footer form-inline">
					<div class="form-group">
						...
						<input type="number" id="cheque" data-close-method="calculateRemain" class="form-control showChequeInputModal"
							value="0" />
		...
		@if (!$invoice)
			...
			$(function () {
				...
				$(document).on('change', 'input.receive-all', function(e){
					e.preventDefault()
					var q = $(this).closest("input.input-quantity").val();
					if($(this).prop('checked')) $(this).closest("input.input-receive").val(q);
					calculateTotals()
				})
			...
			function calculateTotals(){
				...
				$('#items-table tbody tr').each(function() {
					...
					var amount = (quantity * price_sell) * bonus;
					totalQ += quantity;
					@if (!$bill)
			...
			function Unit(id, item, unit, quantity = 0, price_purchase = 0, price_sell = 0) {
				...
				this.total = function(){
					return (this.quantity * this.price_sell) * bonus;
				}
			}
